    ...
    return wordpos.seek(ls[loc].synsetOffset,ls[loc].pos).then(function(z){
      ...
      if(Math.random()>.9){
        ...
        as=z.def.trim()
          ...
          var thething =(strs.replace(cha,as).replace("_"," "))
...
var arr=[]
  return wordpos.getNouns(s).then(function(b){
    ...
    return wordpos.lookupNoun(cha).then( function(a){
      ...
      var lo=getRandomInt(0,a.length)
      var ax=a[lo].synsetOffset

      return decid(ax,lo,s,cha,arr,1)
      //  console.log(arr)
      // console.log(s.replace(cha,xs) )
      //console.log(a)
    })
...
//  dothis(100).then(function(a){for(var i=0;i<100;i++){takestring(a[i]).then(console.log)}})
// for(var i=0;i<100;i++){
// takestring('telephone picture game').then(function(a) {takestring(a[0]).then(function(b){takestring(b[0]).then(console.log)})})}
